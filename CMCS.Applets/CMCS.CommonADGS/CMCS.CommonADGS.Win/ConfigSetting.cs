...
namespace CMCS.CommonADGS.Win
{
    public partial class ConfigSetting : Form
    {
        ...
        private void FillUIFromConfig()
        {
            ...
            this.ckIsSecretRunning.Checked = appConfig.IsSecretRunning;
            this.ckVerifyBeforeClose.Checked = appConfig.VerifyBeforeClose;
            this.txtServerIp.Text = appConfig.ServerIp;
            if (appConfig.DataExtractType == "0")
        ...
        private void btnBase_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                node = xdoc.SelectSingleNode("root/IsSecretRunning");
                node.InnerText = ckIsSecretRunning.Checked ? "True" : "False";

                node = xdoc.SelectSingleNode("root/VerifyBeforeClose");
                node.InnerText = ckVerifyBeforeClose.Checked ? "True" : "False";

                node = xdoc.SelectSingleNode("root/ServerIp");
                node.InnerText = txtServerIp.Text.Trim();

                node = xdoc.SelectSingleNode("root/ServerPort");
                node.InnerText = txtServerPort.Text.Trim();

                xdoc.Save(ADGSAppConfig.ConfigXmlPath);

                //clientConfiguration.AppIdentifier = txtAppIdentifier.Text.Trim();
                //clientConfiguration.SelfConnStr = txtSelfConnStr.Text.Trim();
                //clientConfiguration.GrabInterval = Convert.ToDouble(txtGrabInterval.Value);
                //clientConfiguration.OracleKeywords = txtOracleKeywords.Text.Trim();
                //clientConfiguration.Startup = ckStartup.Checked;
                //clientConfiguration.IsSecretRunning = ckIsSecretRunning.Checked;
                //clientConfiguration.VerifyBeforeClose = ckVerifyBeforeClose.Checked;
                //clientConfiguration.ServerIp = txtServerIp.Text.Trim();
        ...
        private void Instruments_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                kp = grab.Parameters.Where(a => a.Key == "DataPath").FirstOrDefault();
                if (!String.IsNullOrWhiteSpace(kp.Key))
                    txtDataPath.Text = kp.Value;
            ...
            txtProcessName.Text = grab.ProcessName;
            txtServerPort.Text = grab.Port;
            txtDataPath.Text = grab.DataPath;
        }
        #endregion

        #region 抓取类型切换
        private void extractType_CheckedChanged(object sender, EventArgs e)
        {
            if (extractData.Checked)
            {
                txtSelfConnStr.Enabled = true;
                //txtGrabInterval.Enabled = true;
                txtOracleKeywords.Enabled = true;
            ...
            if (sendData.Checked)
            {
                txtSelfConnStr.Enabled = false;
                //txtGrabInterval.Enabled = false;
                txtOracleKeywords.Enabled = false;
        ...
        private void SaveAssayInstruments()
        {
            ...
            foreach (AssayGraber item in appConfig.AssayGrabers)
            {
                ...
                XmlElement ProcessName = xdoc.CreateElement("Param");
                ProcessName.SetAttribute("Key", "ProcessName");
                ProcessName.SetAttribute("Value", item.ProcessName);
                ...
                if (item.Parameters.ContainsKey("GaberType"))
                {
                    ...
                    selfGracer.AppendChild(ProcessName);
                ...
                else
                {
                    ...
                    sysGracer.AppendChild(ProcessName);
                    ...
                    XmlElement DataPath = xdoc.CreateElement("Param");
                    DataPath.SetAttribute("Key", "DataPath");
                    DataPath.SetAttribute("Value", item.Parameters.Where(a => a.Key == "DataPath").FirstOrDefault().Value);
                    sysGracer.AppendChild(DataPath);

                    XmlElement Port = xdoc.CreateElement("Param");
                    Port.SetAttribute("Key", "Port");
                    Port.SetAttribute("Value", item.Parameters.Where(a => a.Key == "Port").FirstOrDefault().Value);
                    sysGracer.AppendChild(Port);
        ...
        private void btnSaveAssay_Click(object sender, EventArgs e)
        {
            ...
            appConfig.AssayGrabers[index].ProcessName = this.txtProcessName.Text.Trim();
            ...
            if (sysGraber.Checked)
            {
                ...
                if (appConfig.AssayGrabers[index].Parameters.Keys.Contains("DataPath"))
                    appConfig.AssayGrabers[index].Parameters.Remove("DataPath");
                if (appConfig.AssayGrabers[index].Parameters.Keys.Contains("Port"))
                    appConfig.AssayGrabers[index].Parameters.Remove("Port");
                ...
                appConfig.AssayGrabers[index].Parameters.Add("DataPath", txtDataPath.Text.Trim());
                appConfig.AssayGrabers[index].Parameters.Add("Port", txtServerPort.Text);
            }
            ...
            //if (sysGraber.Checked)
            //{
            //    clientConfiguration.ByoGrabers[index].MachineCode = this.txtMachineCode.Text.Trim();
            //    clientConfiguration.ByoGrabers[index].TableName = this.txtTableName.Text.Trim();
            //    clientConfiguration.ByoGrabers[index].PrimaryKeys = this.txtPrimaryKeys.Text.Trim();
            //    clientConfiguration.ByoGrabers[index].ConnStr = this.txtConnStr.Text.Trim();
            //    clientConfiguration.ByoGrabers[index].ProcessName = this.txtProcessName.Text.Trim();
            //    clientConfiguration.ByoGrabers[index].Enabled = ckEnabled.Checked;
            //    clientConfiguration.ByoGrabers[index].DbType = ddlDbType.SelectedItem.ToString();
            //    clientConfiguration.ByoGrabers[index].SQL = txtSQL.Text.Trim();
            //    clientConfiguration.ByoGrabers[index].DataPath = txtDataPath.Text.Trim();
            //}
            //else if (selfDefineGraber.Checked)
            //{
            //    clientConfiguration.AssayGrabers[index].MachineCode = this.txtMachineCode.Text.Trim();
            //    clientConfiguration.AssayGrabers[index].TableName = this.txtTableName.Text.Trim();
            //    clientConfiguration.AssayGrabers[index].PrimaryKeys = this.txtPrimaryKeys.Text.Trim();
            //    clientConfiguration.AssayGrabers[index].ConnStr = this.txtConnStr.Text.Trim();
            //    clientConfiguration.AssayGrabers[index].ProcessName = this.txtProcessName.Text.Trim();
            //    clientConfiguration.AssayGrabers[index].Enabled = ckEnabled.Checked;
            //    clientConfiguration.AssayGrabers[index].GaberType = txtGaberType.Text.Trim();
            //    clientConfiguration.AssayGrabers[index].DayRange = Convert.ToDouble(txtDayRange.Value);
            //}
            try
            {
